---
sidebar_position: 6
---
# Viewing Utilisation

Utilisation reports provide historical usage details for your linux instance. You can view daily maximum, minimum, and average readings across all supported parameters. The utilisation table presents this information in a clear date‑wise format, and you can download the report as a .csv file for further analysis.

To view historical usage across supported parameters, follow these steps:

1. Navigate to **Compute > Linux Instances**. The following screen appears:![Create Linux Instance](img/CreatingLinuxInstances1.png)
2. Click on your created Linux instance name from the list. The following screen appears: ![Viewing Details of Linux Instances](img/LaunchConsole.png)
3. Click **Utilisation**. The following screen appears:
   ![Viewing Utilisation](img/ViewingUtilisation.png)