...
To initiate the firewall creation, follow these steps:

1. Navigate to **Networking > Virtual Firewalls**.
2. Click the **New Virtual Firewall** button.
...
To configure the new virtual firewall in the Ananta cloud platform, you must define a few key options such as the zone, compute pack, firewall provider, and access rules. These settings ensure the firewall is tailored to your deployment needs.

To provide the configuration details, follow these steps:
...
2. **Choose Virtual Firewall Configuration**: Select a compute pack that matches your performance requirements, such as:
    - 2C4R: 2 vCPU, 4 GB RAM
    - 2C8R: 2 vCPU, 8 GB RAM
    - 2C16R: 2 vCPU, 16 GB RAM
    ![firewallnew|697](img/CreatingaVirtualFirewall2.png)
3. **Select Firewall Provider**: Choose a firewall provider from the dropdown (for example: pfsense Virtual Firewall)    
4. **Set Incoming Traffic Rule**: Select the desired ACL rule (Allow All, Deny All) from dropdown.
5. **Name Your Virtual Firewall**: Enter a name for your virtual firewall.
6. **Review Estimated Costs**: Verify the summary and pricing before proceeding.![providertocost](img/FirewallProvider.png)  
### Deploying Virtual Firewall

After completing the configuration and deployment steps, the newly created virtual firewall appears in Virtual Firewalls. 

To access Virtual Firewalls and verify that your newly deployed firewall is active and correctly configured:

1. Navigate to **Networking > Virtual Firewalls**. The following screen appears with list of created virtual firewalls: ![firewalltest](img/CreatingaVirtualFirewall1.png)
...
To viewing the details of your virtual firewall instance, follow these steps:
1. Navigate to **Networking > Virtual Firewalls**. 
2. Click the **Virtual Firewall Name**.
3. The following **Overview** screen appears where you can view the following details:
    - **Configuration**: Displays the details like vCPU, RAM, and OS details.
    - **Availability Zone**: Displays the zone you have selected while creating the virtual firewall for example: GCC Chennai.
    - **Public IPv4 (L2-WAN)**: Displays the public IP which is used for SSH access.
    - **Created**: Displays the time of creation.
...
To understand the network configuration, navigate to the **Networking** tab where you can view both **LAN** and **WAN** tiers. This helps identify how instances are connected and how traffic flows between internal and external networks.

To access the networking tab and identifying the WAN and LAN tiers connected to your firewall, follow these steps:
1. Navigate to **Networking > Virtual Firewalls**. 
2. Click the **Virtual Firewall Name**.
3. Navigate to the **Networking** tab.
-  Identify the **WAN Tier**: Look for the network labeled as **ISOLATED_NETWORK**. This is the WAN Tier, typically connected to external/public networks.
-  Identify the **LAN Tiers**: All networks labeled as **LAN_TIER_X** (where X is a number) represent LAN tiers used for internal communications. 
...
To access the pfsense firewall GUI from your browser using the public IP address, follow these steps:
...
2. Open a Web Browser, and paste the IP address in the address bar and press **Enter**.
3. **Ignore the Security Warning**: If a warning appears, it is because of a self-signed certificate. Click **Advanced** and then **Proceed** to continue.
4. **Accept the License**: A blue screen with license information appears. Click the **Accept** button.
...
To deploy the new Linux instance under the pfsense firewall, follow these steps:
1. Navigate to **Compute > Linux Instance**.
2. Choose Availability Zone: Select a zone for example GCC Chennai (or as per your requirement).
3. Select Compute Configuration: Choose an instance configuration for example 2C4R (2 vCPU, 4 GB RAM).
4. Click **Select** to confirm the compute pack.
5. Select Firewall Provider: Choose pfSense Plan from the dropdown.
6. Select Incoming Traffic Rule: Choose Allow All (or set rules as needed).
7. Name Your Virtual Firewall: Provide a name.
8. After filling in all details, proceed with the deployment.
...
2. The window displays the **Monthly** and **Hourly** charge.
...
3. To check the instance's IP address, run the `ip a` command. 
4. to test internet connectivity, run the `ping 1.1.1.1` command.
